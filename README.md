...
- `POST /bookmarks/{id}/access` - Record a bookmark access (updates last_accessed)
...
Interactive API documentation (Swagger UI) is available at `http://127.0.0.1:<port>/docs` when the server is running.
...
- **API Reference**: Available at `http://127.0.0.1:<port>/docs` (Swagger UI) when server is running
