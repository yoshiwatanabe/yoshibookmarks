...
The server defaults to port 8000 (configurable with `--port`). The URL is displayed on startup.
...
- `POST /bookmarks/{id}/access` - Track bookmark access (updates `last_accessed`)
...
- `GET /ingest/preview/{id}/diagnostics` - Provider trace for a specific preview
